...
-- 멱등: 이미 있으면 무시. UNIQUE 제약은 내부적으로 같은 이름의 인덱스를 만들어,
-- 재실행 시 duplicate_object 가 아니라 duplicate_table(relation already exists)로 실패한다.
-- 따라서 두 예외를 모두 잡는다. 0001 은 수정하지 않는다.
